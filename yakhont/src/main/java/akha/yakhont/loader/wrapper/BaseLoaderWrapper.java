...
import akha.yakhont.loader.BaseConverter;       // for javadoc
...
public abstract class BaseLoaderWrapper<D> {
    ...
    public interface Converter<D> extends CacheHelper {
         ...
         *
         * @param position
         *        The cursor position (or null - to retrieve all data)
         *
         * @return  The data
         */
        Object getData(Cursor cursor, Integer position);
    ...
    public interface CacheHelper {
        ...
        /**
         * Gets the "internal cache" flag.
         *
         * @return  {@code true} if Yakhont internal cache used, {@code false} otherwise
         */
         ...
         */
        @SuppressWarnings("unused")
        Cursor getCursor(String tableName, LoadParameters parameters) throws UnsupportedOperationException;
    }
     ...
     */
    protected BaseLoaderWrapper(@NonNull final ViewModelStoreOwner viewModelStoreOwner, final String loaderId) {
        this(viewModelStoreOwner, loaderId, isLoaderIdAutoGenerated(loaderId));
    }

    /** @exclude */ @SuppressWarnings({"JavaDoc", "WeakerAccess"})
    protected BaseLoaderWrapper(@NonNull final ViewModelStoreOwner viewModelStoreOwner, final String loaderId,
                                final boolean  loaderIdAutoGenerated) {
        this(BaseViewModel.getViewModelStore(viewModelStoreOwner), loaderId, loaderIdAutoGenerated);
    }
     ...
     */
    @SuppressWarnings("WeakerAccess")
    public static boolean start(final ViewModelStoreOwner              viewModelStoreOwner,
    ...
    public boolean start(final ViewModelStoreOwner viewModelStoreOwner, final LoadParameters parameters) {
        final boolean result = startHelper(viewModelStoreOwner, parameters);
        CoreLogger.log(result ? CoreLogger.getDefaultLevel(): Level.ERROR, "start loading result: " + result);
        return result;
    }

    private boolean startHelper(final ViewModelStoreOwner viewModelStoreOwner, final LoadParameters parameters) {
        if (parameters == null)
            return startLoading(viewModelStoreOwner, null);

        if (parameters.getSync())
            return startSync(viewModelStoreOwner, parameters) != null;
        ...
        return startLoading(viewModelStoreOwner, parameters);
    }
    ...
    protected BaseViewModel<D> getBaseViewModel(final Activity activity) {
        ...
        if (activity == null)       // e.g. service
            CoreLogger.logWarning("getBaseViewModel: activity == null");
        else {
    ...
    /**
     * Returns the {@link BaseViewModel} key (please refer to {@link ViewModelProvider#get(String, Class)}
     * for more info).
     *
     * @return  The {@link BaseViewModel} key
     */
    ...
    private class RequesterHelper implements DataLoader<D> {
        ...
        private Converter<?> getConverterHelper() {
            ...
            if (converter == null)
                CoreLogger.logWarning("converter == null, loader ID: " + mLoaderId);
            return converter;
        }
        ...
        private abstract class CallableHelper {
            protected abstract boolean call(final Converter<?> converter);
        }
        ...
        public Boolean clear(final String tableName) {
            return handle(new CallableHelper() {
                @Override
                protected boolean call(final Converter<?> converter) {
                    return converter.clear(tableName);
                }
            });
        }
        ...
        public Cursor getCursor(final String tableName, final LoadParameters parameters) throws UnsupportedOperationException {
            final Converter<?> converter = getConverterHelper();
            if (converter == null)
                throw new UnsupportedOperationException("RequesterHelper.getCursor(" + tableName + ")");
            return converter.getCursor(tableName, mParameters);
        }
            ...
            if (model == null)
                ...
                if (coreLoads != null && coreLoads.size() > 0) {
                    ...
                    for (final CoreLoad<?, ?> coreLoad: coreLoads)
                        coreLoad.cancelLoading(activity);
                    return;
                }
                else
    ...
    /**
     * Finds ViewModel (if any) for the given ViewModelStoreOwner.
     *
     * @param viewModelStoreOwner
     *        The ViewModelStoreOwner
     *
     ...
     */
    @SuppressWarnings("WeakerAccess")
    public BaseViewModel<?> findViewModel(final ViewModelStoreOwner viewModelStoreOwner) {
        return findViewModel(getModels(viewModelStoreOwner));
    }

    private BaseViewModel<?> findViewModel(
            final Set<Map.Entry<String, WeakReference<? extends BaseViewModel<?>>>> viewModels) {

        if (viewModels == null) {
            CoreLogger.log("ViewModels cache == null");
            return null;
        }
        ...
        for (final Map.Entry<String, WeakReference<? extends BaseViewModel<?>>> entry: viewModels) {
            ...
            if (entry.getKey().equals(key)) {
                ...
                CoreLogger.logWarning("model == null, key: " + key);
                break;
            }
        }
        return null;
    ...
    private boolean startLoading(final ViewModelStoreOwner viewModelStoreOwner, final LoadParameters parameters) {
        final Callable<Boolean> callable = new Callable<Boolean>() {
            @Override
            public Boolean call() {
                return startLoadingHelper(viewModelStoreOwner, parameters);
            }

            @NonNull
            @Override
            public String toString() {
                return "startLoadingHelper()";
            }
        };
        if (Utils.isCurrentThreadMain()) return Utils.safeRunBoolean(callable);
        ...
        Utils.postToMainLoop(new Runnable() {
            @Override
            public void run() {
                result[0] = Utils.safeRunBoolean(callable);
    ...
    private boolean startLoadingHelper(final ViewModelStoreOwner viewModelStoreOwner, LoadParameters parameters) {
        if (viewModelStoreOwner == null) {
            CoreLogger.logError("startLoading: ViewModelStoreOwner == null");
            return false;
        }
        if (parameters == null) parameters = new LoadParameters();

        final boolean  isActivity = viewModelStoreOwner instanceof Activity;
        final Activity   activity = isActivity ? (Activity) viewModelStoreOwner: null;

        BaseViewModel<?>    model = findViewModel(getModels(viewModelStoreOwner));

        if (model != null && parameters.getNoLoad() && model.isNoLifecycleOwner()) {
            setValue(model);
        ...
        if (model != null) {
            if (!parameters.getNoLoad()) {
                ...
                mParameters.setTableName(getTableName());

                if (isActivity && parameters.getPullToRefresh() && mSwipeToRefreshCallback != null)
                    Utils.safeRun(mSwipeToRefreshCallback);
        ...
        CoreLogger.logError("there's no model with key: " + getKey());  // should never happen
        return false;
    }
     ...
     */
    @SuppressWarnings("WeakerAccess")
    public boolean invalidateDataSource(final Activity activity) {
    ...
    @SuppressWarnings("WeakerAccess")
    @WorkerThread
    public D startSync(final ViewModelStoreOwner viewModelStoreOwner, final LoadParameters parameters) {
        final boolean result = startSync(viewModelStoreOwner, Collections.singletonList(this), parameters);
    ...
    @SuppressWarnings("WeakerAccess")
    @WorkerThread
    public static boolean startSync(final ViewModelStoreOwner              viewModelStoreOwner,
                                    final Collection<BaseLoaderWrapper<?>> loaders,
                                    final LoadParameters                   parameters) {
        if (viewModelStoreOwner == null) {
            CoreLogger.logError("startSync: ViewModelStoreOwner == null");
            return false;
        }
     ...
     */
    @SuppressWarnings("WeakerAccess")
    public static class SwipeToRefreshWrapper {
        ...
        public static <D> boolean register(Activity activity, final SwipeRefreshLayout swipeRefreshLayout,
            ...
            final SwipeToRefreshWrapper swipeToRefreshWrapper  = new SwipeToRefreshWrapper(
        ...
        private static class OnRefreshListenerHelper<D> implements OnRefreshListener {
            ...
            public void onRefresh() {
                ...
                // in case of paging
                if (mBaseLoaderWrapper.invalidateDataSource(Utils.getCurrentActivity())) {
                ...
                mBaseLoaderWrapper.start(BaseViewModel.cast(Utils.getCurrentActivity(), null),
                        new LoadParameters(loadParameters, true));
            }
        }
    ...
    public static class LoadParameters implements Parcelable {
        ...
        private LoadParameters(final Parcel in) {
            ...
            mTimeout       = in.readInt();
            mPageId        = in.readLong();
            mStore         = (DataStore) in.readSerializable();
        }
        ...
        public void writeToParcel(final Parcel out, final int flags) {
            ...
            out.writeSerializable(mStore);
        }

        /**
         * Sets some data to keep in this {@link LoadParameters}.
         *
         * @param key
         *        The key
         *
        ...
        /**
         * Returns the data (associated with the given key) kept in this {@link LoadParameters}.
         *
         * @param key
         *        The key
         *
         ...
         *
         * @param pageId
         *        The page ID (if paging adapter used) or null
         */
        @SuppressWarnings("unused")
        public LoadParameters(final Long pageId) {
            this(null, null, false, false, false,
                    false, false, false, false, false, pageId);
        }

        private LoadParameters(final boolean noLoad) {
            this(null, null, false, false, false,
                    false, false, false, false, noLoad, null);
        }
        ...
        public LoadParameters(@NonNull final LoadParameters loadParameters, final boolean pullToRefresh) {
            this(loadParameters.mLoaderId, loadParameters.mTimeout, loadParameters.mForceCache,
                    ...
                    loadParameters.mNoLoad, loadParameters.mPageId);
            setTableName(loadParameters.mTableName);
        }
        ...
        private LoadParameters(final String  loaderId,   final Integer timeout, final boolean forceCache,
                               ...
                               final boolean sync, final boolean handleTimeout, final boolean pullToRefresh,
                               final boolean noLoad,     final Long pageId) {
            mLoaderId       = loaderId;
            ...
            CoreLogger.log("new LoadParameters - loader ID: " + loaderId +
                    ...
                    ", merge: " + merge + ", no errors: " + noErrors + ", sync: " + sync +
                    ", handle timeout: " + handleTimeout + ", pull-to-refresh: " + mPullToRefresh +
                    ", no load: " + mNoLoad + ", timeout (ms): " + mTimeout + ", page ID: " + mPageId);
        ...
        /**
         * Sets the cache table name.
         *
         * @param tableName
         *        The cache table name
         *
         * @return  This {@code LoadParameters} object
         */
        @SuppressWarnings({"unused", "WeakerAccess"})
        public LoadParameters setTableName(final String tableName) {
        ...
        public String toString() {
            return String.format(Utils.getLocale(), "force cache %b, no progress %b, " +
                            ...
                            "table name %s, page ID %d",
                    mForceCache, mNoProgress, mNoErrors, mMerge, mSync, mHandleTimeout, mError,
                    mLoaderId, mTimeout, mPullToRefresh, mNoLoad, mTableName, mPageId);
        }
        ...
        public static class Builder {
            ...
            /**
             * Sets the page ID (if paging adapter used).
             *
             * @param pageId
             *        The page ID (or null)
             *
             ...
             */
            @SuppressWarnings({"unused", "WeakerAccess"})
            public Builder setPageId(final Long pageId) {
                mPageId         = pageId;
                return this;
            }
